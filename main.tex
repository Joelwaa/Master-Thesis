...
\end{proof}
Note that the argument in \cref{eq:concluding monotone class for discriminatory functions} can be shortened under ruling assumptions. Indeed, we assumed \(\exists \nu\neq 0\) such that \(\int_X\psi(f(x)+\beta)\nu(\diff x)\quad\forall f\in X^*, \beta\in\mathbb{R}\). In particular, \(f=0\quad\forall x\in X\) and \(\beta = 0\) are admissible; hence \(0=\int_X\psi(0+0)\nu(\diff x)=\psi(0)\nu(X)\implies \psi(0)=0\). Hence 
\begin{equation}
...
the previous section no longer applies directly.
 For the following, we essentially follow \parencite{Schweizer2001GuidedTourQuadraticHedging}. We will not give proofs for the following results as they are all nicely presented in \parencite[Chapter.~4]{Schweizer2001GuidedTourQuadraticHedging}.
 \bigbreak
...
\end{proposition}
\begin{proof}
    The density of \(\widetilde{\mathcal{E}}(\psi)\) in \(L^2(M)\) analogoues to \cref{prop:Lp(M) density of E(psi) under compacteness}. The last claim follows the exact same reasoning as in \cref{prop:Lp(M) density of E(psi)}.
\end{proof}
...
\end{equation}
and \(\lVert H\rVert_{L^2(M,A)}^2=\lVert H\rVert_{L^2(A)}+\lVert H\rVert_{L^2(M)}\). As usual we will abuse notation in talking about elements in \(L^2(M,A)\) as processes although, strictly speaking, it containes equivalence classes of processes. 
...
We thus find that condition \(D_2(P)\) holds for bounded mean-variance tradeoff.  The following condition is common in the literature
\begin{definition}\label{def:condition SC_stro}
    ...
    \end{itemize}
\end{definition}
Note that assuming bounded MVP in particular implies the \(SC\) condition. Indeed \(\langle M\rangle\) is a non-decreasing process and \(\lambda^2\geq 0\) hence \(K_t=(\lambda\bullet X)_t\) is non-decreasing. 
...
\begin{proof}
    ...
    \\\\
   Hence, the 'if' part is a direct application of the inverse operator Theorem. Conversely, we know that \(L^2(M,A)\) is Banach, and by the previous Lemma, \(I_{\infty}\) is a continuous, bijective, linear map. If condition \cref{eq:homeo condition} holds, it is a homeomorphsim and in particular a closed map. That is \(I_{\infty}(L^2(M,A))=G_T(L^2(M,A))\) is closed as the image of a closed space under a closed map. 
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        \lVert G_T(\epsilon\vartheta)\rVert_{\mathcal{L}^2}&\leq \lVert \epsilon\vartheta\rVert_{L^2(M)}+\lVert \epsilon\vartheta\rVert_{L^2(A)}\\
        &\leq \lVert \vartheta\rVert_{L^2(M)}+\lVert \vartheta\rVert_{L^2(A)}
        \\&= \lVert \vartheta\rVert_{L^2(M)}+\eta\;,
    \end{align*}
...
\bigbreak
\textcolor{blue}{discuss Theorem 3.7. Then either cite or proof the D2(P) iff bmo2 statement. Finally cite the R2(P) condition and according necessary and sufficient conditions}
\bigbreak
...
\end{proof}
According to \cref{prop:Lp(M) density of E(psi) under compacteness}, considering the market information process \(\widetilde{Y}=\tanh(Y)\) instead of \(Y\), effectively replacing \(\mathcal{E}(\psi)\) by \(\widetilde{\mathcal{E}}(\psi)\) we can let go of the additional \(\epsilon\)-integrability condition on \(M\). Furthermore, according to \parencite[Thm.~3.7]{DelbaenSchachermayer1998ExistenceAbsolutelyContinuousLMM} the assumptions in \cref{prop:continuous time deep mean variance hedging for semimartingales} already imply the \(bmo_2\)-condition, which can be dropped. We get the following version. 
...
\end{equation}
Hence \(\mathcal{P}(A,|A|)\) is bounded in \(L^0\) and thus there exists a predictable density \(\alpha_s\). Furthermore 
\begin{equation}
    dA_s = \alpha_s\diff D_s
\end{equation}
implies that \(|\alpha_s|=1\). Hence
\begin{equation}
    \alpha_s\epsilon_s
\end{equation}
is predictable and takes values in \(\left\{-1,1\right\}\). Finally let
\begin{equation}
    sgn(\alpha\theta)_s\coloneqq signum(\alpha_s\theta_s)=1_{\alpha_s\theta_s>0}-1_{\alpha_s\theta_s\leq 0}\;.
\end{equation}
Note that \(sgn(\alpha_s\theta_s)\) is predictable as linear combination of indicators of predictable sets  (because \(\alpha\) is predictable and \(\vartheta\in L^2(M)\cap L^2(A)\) as well). Hence 
\begin{equation}
    \theta\bullet A=sgn(\theta_s\alpha_s)\theta_s\alpha_s\bullet |A|
\end{equation}
is non-decreasing. Does if \(\theta\bullet A\) is not non-decreasing, apply the theorem to \(\theta\alpha\bullet A\). The conclusion remainse the same for the predictable process \(\widetilde{\epsilon}=\epsilon\alpha\) which takes values in \(\left\{-1,1\right\}\).
\end{appendices}
\end{document}
