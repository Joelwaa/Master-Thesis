...
\end{equation}
where \(f_k:\mathbb{R}^{r(k+1)}\to\mathbb{R}^d\) is Borel-measurable.
\\
Notice that in our current setting, finite \(\Omega\) we have that 
\begin{equation}
    \mathbb{E}_{\mathbb{P}}\left[f_k(Y_0,\dots,Y_k)\right]=\sum_{\omega\in\Omega}f_k(Y_0(\omega),\dots,Y_k(\omega))<\infty\;,
\end{equation}
is finite as a finite sum over finite terms.
In particular, due to
\begin{equation}
    \mathbb{E}_{\mathbb{P}}\left[f_k(Y_0,\dots,Y_k)\right]=\mathbb{E}_{\mathcal{L}(Y_0,\dots,Y_k)}\left[f_k\right]\;,
\end{equation}
where \(\mathcal{L}(Y_0,\dots,Y_k)\) denotes the law of \((Y_0,\dots,Y_k)\)\footnote{That is, the push-forward measure \(\mathbb{P}_*(Y_0,\dots,Y_k)\coloneqq\mathbb{P}\circ(Y_0,\dots,Y_k)^{-1}\).}, we have 
\begin{equation}\label{eq:f is in L1 for finite Omega}
    f_k\in L^1(\mathbb{R}^{r(k+1)},\mathcal{L}(Y_0,\dots,Y_k))\;.
\end{equation}
More generally, we have
\begin{equation}
    f_k\in L^p(\mathbb{R}^{r(k+1)},\mathcal{L}(Y_0,\dots,Y_k))\quad\forall p>0\;.
\end{equation}
In the probabilistic case, measure spaces are finite (probability spaces); hence \(L^p\subset L^q\) for \(p\geq q\). Hence, we need an approximation system \(\Psi\) that approximates functions from \(\subset\  L^1(\mathbb{R}^{r(k+1)},\mathcal{L}(Y_0,\dots,Y_k))\) arbitrarily well. Technically, we want the following density statement.
\begin{equation}
    \overline{\Psi}^{L^1}=L^1(\mathbb{R}^{r(k+1)},\mathcal{L}(Y_0,\dots,Y_k))\;.
\end{equation}
...
\begin{equation}\label{eq:restricted strategies}
    \Theta_M\coloneqq\{\vartheta=(\vartheta_k)_{k=0}^{n-1}\in\Theta:\vartheta_k=F_k(Y_0,\dots,Y_k,\vartheta_{k-1}),\quad F_k\in\Psi_M\}\;.
\end{equation}
Notice the analogy with the Doob-Dynkin representation of the strategy \(\vartheta\). In this sense, \(\Theta_M\) is the set of strategies that have a Doob-Dynkin representation in \(\Psi_M\). We can now state the restricted optimization problem. 
...
\begin{proof}
 ...
 \end{equation}
 Consider this \(\vartheta=(\vartheta_k)_{K=0}^{n-1}\). By definition, \(\vartheta_k\) is \(\mathcal{F}_k\)-measurable. So, by the Doob-Dynking lemma, there exists \(f_k:\mathbb{R}^{r(k+1)}\to\mathbb{R}^d\) measurable such that 
 \begin{equation}
     \vartheta_k=f_k(Y_0,\dots,Y_k)\;,
 \end{equation}
 for all \(k=0,\dots,n-1\). As argued in \cref{eq:f is in L1 for finite Omega} \(f_k\in L^1(\mathbb{R}^{r(k+1)},\mathcal{L}(Y_0,\dots,Y_k))\) because \(\Omega\) is finite. Let's denote \(\mu\coloneqq\mathcal{L}(Y_0,\dots,Y_k)\). By the density of \(\Psi\) in \(L^1\), there exists a sequence of functions \(F_k^n\) converging to \(f_k\) in \(L^1(\mathbb{R}^{r(k+1)},\mu;\mathbb{R}^d)\) or equivalently \(\vartheta_k^n\coloneqq F_k^n(Y_0,\dots,Y_k)\) converges to \(f_k(Y_0,\dots,Y_k)\) in \(L^1(\Omega,\mathbb{P};\mathbb{R}^d)\)\footnote{in the sense that \(\int_{\mathbb{R}^{r(k+1)}}\lVert f_k-F_k^n\rVert_{d}\diff \mu\to 0\) as \(n\to\infty\), where \(\lVert\cdot\lVert_d\) denotes the Euclidean norm on \(\mathbb{R}^d\)}. Passing to a subsequence, we get \(\mathbb{P}\)-almost sure convergence. Morally speaking, we want to pass the limit \(n\to\infty\) inside \(\rho\). Even if \(\rho\) has sufficient regularity, \(\mathbb{P}\)-a.s. convergence is not enough to get the claim. Lukely we are in the finite setting, hence \(\mathbb{P}(\{\omega\})>0\) for all \(\omega\in\Omega\), hence \(\mathbb{P}\)-almost sure convergence is equivalent to convergence everywhere. Thus
 \begin{equation}
 ...
 In particular, 
 \begin{align}
(\vartheta^n\bullet X)_T(\omega)=\sum_{k=0}^{n-1}\vartheta^n_k(\omega)\cdot(X_{k+1}(\omega)-X_k(\omega))\overset{n\to\infty}{\rightarrow}(\vartheta\bullet X)_T(\omega)\;.
\end{align}
...
\begin{proposition}[Finite Deep Mean variance Hedging]\label{prop:finite deep mean variance hedging}
    \begin{equation}
        \inf_{\vartheta\in\Theta}\mathbb{E}[\left(H-V_0-(\vartheta\bullet X)_T\right)^2]=\inf_{\vartheta\in\Psi}\mathbb{E}[\left(H-V_0-(\vartheta\bullet X)_T\right)^2].
    \end{equation}
\end{proposition}
