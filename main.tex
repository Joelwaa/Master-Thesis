...
\end{equation}
and \(\lVert H\rVert_{L^2(M,A)}^2=\lVert H\rVert_{L^2(A)}+\lVert H\rVert_{L^2(M)}\). Also it is clear by construction that 
\(\Theta=L^2(M,A)\subset \Theta^2\).
 Hence, for processes \(H\in L^2(A,M)\), we have that the map
...
\end{equation}
For this, let's take a step back to our general considerations. Recall that we were naturally led to consider the map \cref{eq:final integral map} 
\begin{equation}
...
\end{proof}
Hence \(I_{\infty}\) is a continuous bijective map from the Banach space \(L^2(M,A)\) to \(G_T(L^2(M,A))\subset L^2(\mathcal{F}_{\infty},P)\). The crucial observation is that \(I_{\infty}\) is a homeomorphism if and only if \(G_T(L^2(M,A))\) is a Banach space, that is in particular a closed subsapce of \(L^2(P)\). In otherwords
\begin{proposition}
    \(G_T(L^2(M,A))\subset L
\mathcal{L}^2(\mathcal{F}_{\infty},P)\) if and only if there exists \(C>0\) such that 
\begin{equation}
    \forall\vartheta\in L^2(M,A), \quad \lVert \vartheta\rVert_{L^2(M,A)}\leq C\lVert G_T(\theta)\rVert_{\mathcal{L}^2(P)}\,.
\end{equation}
\end{proposition}
\begin{proof}
    This follows directly from the Inverse Operator Theorem, (see ....) that states that every bijective bounded linear operator \(T:X\to Y\) between two Banach spaces \(X,Y\) is a homeomorphism, which means that the inverse map is also continuous. As the inverse of a linear map is necessarily linear, the claim that the inverse is continuous is equivalent to the claim that the inverse is bounded. 
\end{proof}
This yields a functinal analytic, sufficient and necessary condition for the closedenss of \(G_T(L^2(M,A))\) and hence the existence of the mean-variance hedging solution.  Interestingly the this will lead us to discovering a connection betweent the clodeness of \(G_T(L^2(M,A))\) and the condition \(D_2(P)\). 
\bigbreak
...
\bigbreak
In particular, \(L^2(M,A),\lVert\cdot\rVert_{L^2(M,A)})\) is a Banach space. We want to show that the image of \(L^2(M,A)\) under \((I_{\infty}\) is closed. 
We find a sufficient answer to this in \parencite[Thm.4.1]{DelbaenSchachermayer1998ExistenceAbsolutelyContinuousLMM}. We restate the relevant part for convenience. But to understand the statement we need to following definition.
...
\]
For \(p=+\infty\) we require that \(\frac{L_T}{L_t}\) is bounded by \(C\). We say that an ELMM \(Q\) satisfies \(\mathcal{R}_p(P)\) if the respective density process 
\begin{equation}
    Z_t=\mathbb{E}[\frac{\diff Q}{\diff P}|\mathcal{F}_t]
\end{equation}
satisfies \(\mathcal{R}_p(P)\).\\\\

The relevant part of  \parencite[Thm.4.1]{DelbaenSchachermayer1998ExistenceAbsolutelyContinuousLMM} is as follows:
...
Thus, to have an existence claim about the mean-variance hedging solution we require \(X\) to be continuous. We can finally state an extension of \cref{thm:martingale setup, continuous deep variance hedging} to price processes in \(L^2(M,A)\) 
...
To state necessary and sufficient conditions for \(D_2(P)\) we need to introduce the space of \(bmo_2\) local-martingales
\begin{definition}
...
\begin{proposition}\label{prop:continuous time deep mean variance hedging for semimartingales}
    Assume the \(X=X_0+M+A\) is a continuous semimartingale in \(\mathcal{S}_{loc}^2\), such that \(\mathbb{P}^2_e(X)\neq \emptyset\) and  \(\exists \epsilon>0\) for which \(M\in\mathcal{H}^{2+\epsilon}\) and assume there exists a predictable process \(\lambda\) such that \(\diff A=\lambda\diff \langle M\rangle\) and \(\lambda\bullet X\in bmo_2\). Furthermore assume that there is at least one \(Q\in\mathbb{P}^2_e(X)\) that satisfies \(\mathcal{R}_2(P)\). Let \(H\in L^2(\mathbb{P})\) be a real-valued \(\mathcal{F}_{\infty}\)-measurable random variable. Furthermore, assume \(\mathcal{F}_0\) to be trivial. Then:
\begin{equation}\label{eq:martingale setup, continuous deep variance hedging}
    \min_{c\in\mathbb{R},\,\vartheta\in L^2(M,A)}\mathbb{E}\Bigl[\bigl(H-c-\int_{0}^T\vartheta_t\diff X_t\bigr)^2\Bigr]
    \,=\,\inf_{c\in\mathbb{R},\,\vartheta\in \mathcal{E}(\psi)}\mathbb{E}\Bigl[\bigl(H-c-\int_{0}^T\vartheta_t\diff X_t\bigr)^2\Bigr]\;,
\end{equation}
\end{proposition}
\begin{proof}
The existence follows as in \cref{prop:deep mean variance for continuous semimartingales}. The equality follows from density of \(\mathcal{E}(\psi)\) in \(L^2(M)=L^2(M,A)\) due to \(X\in bmo_2\).
\end{proof}
...
\end{proposition}
\begin{proof}
    This version uses that according to \parencite[Thm.~3.7]{DelbaenSchachermayer1998ExistenceAbsolutelyContinuousLMM}, if \(G_T(\Theta)\) is closed in  \(\mathcal{L}^2(P)\), \(A\) is continuous and \(Q\in\mathbb{P}^2_e(X)\), then \(D_2(P)\) holds. Hence, \(L^2(M,A)=L^2(M)\) and the mean-variance solutions exists. 
\end{proof}
Recall that \(D_2(P)\) holds if and only if \(\exists\lambda\) and \(\lambda\bullet X\in bmo_2\). Thus the \(bmo_2\)-condition from the previous version still holds. 
\bigbreak
