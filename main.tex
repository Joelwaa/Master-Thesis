...
\end{equation} 
Then \(\mathbb{F}^0\) and  \(\mathbb{F}\) denote the raw and completed filtration respectively generated by the market information \(Y\).
\\\\
...
\begin{align}\label{eq:H^p identification}
    ...
    &M\mapsto M_{\infty}
\end{align}
First of all, because the processes in \(\mathcal{H}^p\) are uniformly integrable, the map is well defined. Injectivity is clear, and surjectivity follows by setting \(M_t = \mathbb{E}[X|\mathcal{F}_t]\) for \(X\in L^p(\Omega,\mathcal{F}_{\infty},\mathbb{P})\). Moreover, \cref{lemma:order of H^p spaces} (2) states that the norm on \(\mathcal{H}^p\) is equivalent to the norm on \(L^p\) under the stated identification. In particular, this implies that the dual of \(\mathcal{H}^p\) can be \(\mathcal{H}^q\) where \(p,q\) are Hölder conjugates. More explicitly, any continuous linear form \(\phi\in(\mathcal{H}^p)^*\) can be identified with some \(N\in\mathcal{H}^q\) and written as \(\phi(M)=\mapsto\mathbb{E}[M_{\infty}N_{\infty}]\). 
\\\\
...
\end{equation}
This inner-product structure on \(\mathcal{H}^2\) induces a natural notion of orthogonality. We call \(X,Y\in\mathcal{H}^2\) weakly orthogonal if \((X,Y)_{\mathcal{H}^2}=0\). The geometry of \(\mathcal{H}^2_0\) being a space of martingales is even richer. More precisely, denoting the subspace \(\mathcal{H}_0^2=\left\{X\in\mathcal{H}^2:X_0=0\right\}\), we can define a stronger notion of orthogonality:
\begin{definition}
...
\end{equation}
Hence, strong orthogonality implies the weak orthogonality of \(X,Y\in\mathcal{H}_0^2\). For simplicity, we will call elements in \(\in\mathcal{H}^2\) square integrable martingales. This will be of independent interest in mean variance hedging and is also the core development for stochastic integration of locally bounded predictable processes with respect to general semimartingales \parencite[Remark.~4, Chapter VII]{DellacherieMeyer1982ProbabilitiesPotentialB}. 
\subsection{Stochastic Integration for square integrable Martingales}
The strategy will be as follows: it will mimic classic Lebesgue integration theory but with a few important caveats. In particular, we will define the stochastic integral for a set of simple functions and extend it to a more general case by an isometric embedding. The construction itself will yield some important properties of the resulting integral.
\\\\
As announced, we first define a suitable class of simple integrands for which we can define the stochastic integral with respect to \(X\in\mathcal{H}^2\) naturally. 
\begin{definition}\label{def:elementary predictable process}
    ...
    \end{equation}
\end{definition}
In particular, we have that \(H^i\) is \(\mathcal{F}_{t_i}\)-measurable. We denote the set of elementary predictable processes by \(\mathcal{E}\). Notice that we define the elementary predictable processes as having left-continuous paths.
...
\bigbreak
Given a process \(X\in\mathcal{H}^2\) we define the stochastic integral \(I:\mathcal{E}\to\mathcal{H}^2\)
as the stochastic process
\begin{equation}
   \int H_s\diff X_s =\left(\int_0^t H_s\diff X_s\right)_{t\in\mathbb{R}_+}
\end{equation}
where
\begin{equation}\label{eq:integral of simple processes}
    \int_0^t H_s\diff\coloneqq\sum_{i=1}^nH^i(X_{t_{i+1}\wedge t}-X_{t_i\wedge t})  
\end{equation}
For \(X\in\mathcal{H}^2\) one can define its quadratic variation process \(\langle X\rangle\) (see \parencite[Def.11.1.1.]{CohenElliott2015StochasticCalculusApplications}. The crucial observation is the following.
\begin{proposition}\label{prop:Ito isometry for simple processes}
    For \(H\in\mathcal{E}\) and \(X\in\mathcal{H}_0^2\), we have that \(\int H\diff X\in\mathcal{H}^2\) and the property
...
\bigbreak
We first make a crucial observation, justifying the choice of notation \(L^2(X)\). Defining the measure \(\mathbb{P}_M\coloneqq\mathbb{P}\otimes \langle M\rangle\) on the product space \(\Omega\times [0,\infty)\) we have by definition \cref{def:L^2(M)}:
...
\begin{lemma}\label{lemma:elementary processes generated predictable sigma algebra}
    ...
    \end{equation}
    \begin{proof}
        By definition, \(\mathcal{P}\) is generated by the family of adapted, left-continuous processes. Every element of \(\mathcal{E}\) is clearly adapted and left-continuous hence \(\sigma(\mathcal{E})\subset\mathcal{P}\) is clear. Hence it remainse to show that \(\mathcal{P}\subset\sigma(\mathcal{E})\). For this we  first show that any adapted left-continuous process is the limit of elements of \(\mathcal{E}^{\infty}\). Let \(X\) be an adapted left-continuous process. Then define
        \begin{equation}
            X^n= X_01_{\left\{0\right\}}+\sum_{k\geq 1}X_{k/n}1_{(k/n,(k+1)/n]}
        \end{equation}
        As \(X\) is adapted, \(X_{k/n}\) is \(\mathcal{F}_{k/n}\)-measurable, hence indeed \(X^n\in\mathcal{E}^{\infty}\). Then for every fixed \(t\in[0,\infty)\) there exists a unique \(k(n)\in\mathbb{N}\) such that \(\frac{k(n)}{n}<t\leq \frac{k(n)+1}{n}\). Hence \(X_t^n=X_{k(n)/n}\), thus \(X_t^n\) is \(\mathcal{F}_{k/n}\subset\mathcal{F}_t\)-measurable, hence adapted. Furthermore, notice that \(\lim_{n\to\infty}\frac{k(n)}{n}\leq\lim_{n\to\infty}|\frac{k(n)}{n}-t|+t\leq\lim_{n\to\infty}\frac{1}{n}+t=t\) and \(\frac{k(n)}{n}<t\) hence \(k(n)\nearrow t\), hence for every \(t\in[0,\infty),\;X^n_t\to X_t\) as \(n\to\infty\) by left-continuity of \(X\). This implies 
        \begin{equation}
        ...
        \begin{equation}
            \mathcal{P}\subset\mathcal{E}^{\infty}\subset\mathcal{E}\;.
        \end{equation}
        \end{proof}
\end{lemma}
For the application of the monotone class theorem, the following observation will be useful.
...
\begin{proof}
    ...
    \end{equation}
    Then it is clear that \(X^n\to X\) pointwise as \(n\to \infty\), hence the claim follows. 
\end{proof}
...
\begin{theorem}\label{thm:square integrable stochastic integral}
    The linear map \(H\mapsto \int H\diff M\) fo \(\mathcal{E}\) into \(\mathcal{H}_0^2\) extends uniquely to an isometric linear map \(L^2(M)\mapsto\mathcal{H}_0^2\)
\end{theorem}

\textcolor{blue}{Explain weak and strong orthogonaility, need to understand K.W. Watanabe decomposition which will be used to understnad the mean variance hedging solutino in the square integrable case}
...
\end{equation}
 ...
 \bigbreak
 \textcolor{blue}{
 A key difference between the framework in \parencite{Buehler2018DeepHedging} and the continuous time generalization of \parencite[chapter~5]{Arandjelovic2025} relies on the fact that the \(\sigma\)-algebra generated by the market information process \(Y\) is, trivially, finitely generated in the finite case and uncountably generated in the general case. \textcolor{blue}{Might change: "Regarding the route taken in proving \parencite[Porp.~4.9.]{Buehler2018DeepHedging}, this leads to issues in measurability, in particular the application of Doob-Dynkin."} One key insight is that in the case where a random variable takes values in a measurable space with a countably generated \(\sigma\)-algebra, the measurability with respect to an uncountably generated \(\sigma\)-algebra can be reduced to measurability with respect to a countable one. The following proposition states this fact in more general and more precise terms}
...
\end{proof}
\cref{prop:reduction to countable measurability with filtration} now motivates the following definition, which will be the building block for our approximation theory of simple bounded predictable processes based on neural networks. 
\begin{definition}\label{def:algorithmic strategies}
    \begin{itemize}
        ...
        \begin{equation}
            \vartheta = f(\tau,Y_{t_1},\dots,Y_{t_n})\;.
        \end{equation}
    \end{itemize}
\end{definition}
We now turn to the main approximation theoretic result, upon which the rest will be based. We will show that the algorithmically generated random variables are dense in \(\mathcal{L}^p(\Omega,\overline{\mathcal{F}},\mathbb{P})\) for \(p\in[0,\infty)\), where \(\overline{\mathcal{F}}=\sigma(Y_s:0\leq s\leq t)\). As expected from the finite setting \parencite{Buehler2018DeepHedging} the proof will consist of applying the Doob-Dynkin representation theorem
...
\begin{theorem}[Approximation of Stochastic Integrals]\label{thm:Approx of stoch. Integrals}
    Let \(X\) be a càdlàg semimartingale, and \(V\in \mathcal{L}\). Then, there exists a sequence \((V^n)_{n\in\mathbb{N}}\) in \(\mathcal{S}(\psi)\) that converges to \(V\) in \(\mathbb{P}\)-ucp, such that
    \begin{equation}
