...
Explain and motivate why we restrict ourselves to the quadratic risk criterion for the continuous time setup. In particular, explain the \(L^2\) projection viewpoint again.
...
Via Kunita Watanabe. Especially follow Rheinländer-Sexton. Also explain heuristically the construction of the stochastic integral via Itô's Isometry in this case.
...
\begin{align*}
&\lVert H\rVert_{L^2(M,A)}^2\leq\mathbb{E}[\int_0^\infty (H_s)^2\diff [M]_s]+\mathbb{E}[\left(\int_0^\infty |H_s||\lambda_s|\diff[M]_s\right)^2]\\&\leq\mathbb{E}[\int_0^{\infty}(H_s)^2\diff [M]_s\cdot\int_0^{\infty}(\lambda_s)^2\diff [M]_s]
\\&
...
\begin{equation}
    \lVert H\rVert_{L^2(M,A)}\leq\lVert H\rVert_{L^2(M,A)}\operatorname*{ess\sup}\left(\int_0^{\infty}(\lambda_s)^2\diff [M]_s\right)\;.
\end{equation}
