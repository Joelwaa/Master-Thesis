...
\\
We will scetch the main ideas and results leading to necessary and sufficient conditions for the existence of the a solution to the mean variance hedging problem.
...
The definition comes naturally when one tries to bound \(\lVert H\bullet X\rVert_{L^2}\), which is useful for our purpose by \cref{rem:sufficient condition for extension}. Indeed
...
\end{proposition}
\begin{proof}
    The existence follows from the fact that \(\mathcal{E}(\psi)\subset L^2(M)\)  and by the observation that in \cref{eq:bounding L^2(M,A) norm under necessary assumption} we have show in particular that under the current conditions
    \begin{equation}
    ...
    \end{equation}
 Hence, under the current assumptions \(\vartheta\in L^2(A)\cap L^2(M)\), in partciular, by construction \((\vartheta\bullet X)_{\infty}\in L^2(\mathbb{P})\) hence by ....(Hilbert projection theorem) the minimum exists. By remark \cref{rem:sufficient condition for extension} and density of \(\mathcal{E}(\psi)\) in \(L^2(M)=L^2(M,A)\) the claim follows. 
\end{proof}
