...
\\\\
1) Find assumptions on \(X\) and \(\vartheta\) to get a version of \parencite[Thm.~5.29.]{Arandjelovic2025} where we have 2-ucp convergence. If we had that, then due to the trivial inequality \(||M_T||_{L^2}\leq||\sup_{0\leq t\leq T}|M_t|||_^{L^2}\), we get 2-ucp convergence implies \(L^2\)-convergence of the endpoints, and as seen before, we get the theorem from this. In particular look at \parencite[Prop.~5.28.]{Arandjelovic2025} and choose Young function \(\Phi(\cdot)=(\cdot)^2\) to get 2-ucp density. We then still need 2-ucp continuous stochastic integral. Need to look up literature for this.
