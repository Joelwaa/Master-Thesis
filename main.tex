...
Hence, \(\mathcal{A}(\Theta)\) defines the space of terminal values of portfolios \((V_0,\vartheta)\in\mathbb{R}\times \Theta\). We fix \(X=(X_0,\dots,X_d)\) once and for all and won't make the dependency of \(\mathcal{A}(\Theta)\) on \(X\) explicit. Furthermore, we will abuse notation and write \((V_0,\vartheta)\in\mathcal{A}(\Theta)\) meaning that \( 
...
In this context, if a solution of the form \((V_0^*,\vartheta^*)\in\mathcal{A}(\Theta)\) exists, we call \(V_0^*\) the \(\Theta\)-mean-variance price and \(\vartheta^*\) the \(\Theta\)-mean-variance strategy. We might not always make the dependency on \(\Theta\) explicit if it is is clear from the context.
...
\textcolor{blue}{maybe present corollary 3 of chapter II Protter.}
...
\bigbreak
We now turn to the integration for \(M\) a square integrable martingale. The key observation is the so called \textit{Itô Isometry} , which we state below. 
...
\begin{definition}\label{def:L^2(M)}
    ...
    \end{equation}
\end{definition}
For later use we define the general \(L^p(M)\)-space
\begin{definition}\label{def:L^p(M)}
    ...
    \end{equation}
\end{definition}
The definition of \(L^p(M)\) for \(p=2\) reduces to the definition for \(L^2(M)\). \\\\
...
\begin{equation}\label{def:semimartingale}
    X = X_0+M+A\;
\end{equation}
with \(M\in\mathcal{M}_{0,loc}\) and \(A\in FV\). We call \(X\) special if we can choose \(A\) in \cref{def:semimartingale} to be predictable. In that case the decomposition \(X=X_0+M+A\) is unique and refered to as the canonical decomposition of \(X\). In particular we will restrict to the following space of semimartingales.
\begin{definition}
...
\begin{proof}
    ...
    \begin{align*}
        \lVert G_T(\epsilon\vartheta)\rVert_{L^2}&\leq \lVert (\epsilon\vartheta\bullet M)_T\rVert_{L^2}+\lVert (\epsilon\vartheta\bullet M)_T\rVert_{L^2}
        \\
