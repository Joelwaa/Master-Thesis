...
\begin{equation}\label{eq:semimartingale decomposition}
    X=X_0+M+A\;,
\end{equation}
where \(M\in\mathcal{H}^2_{0,loc}\): local martingale starting at \(0\) and \(A\in FV_{0}\): finite variation process starting at \(0\). The existence of a solution to the mean variance hedging problem is a separate, very deep issue that we will rely on but won't discuss in detail (see \parencite{Schweizer2001GuidedTourQuadraticHedging} for a good overview and \parencite{Schweizer1994ApproximatingRandomVariablesStochasticIntegrals} for the exhaustive treatment). By \cref{rem:sufficient condition for extension} we are naturally led to consider the following space
\begin{definition}
Let \(X\) be a semimartingale with representation \cref{eq:semimartingale decomposition}. Then we define the following
\begin{align*}
     \mathcal{L}(M,A)\coloneqq\left\{H \text{ predictable}:\mathbb{E}[\int_0^\infty H_s^2\diff [M]_s]+\mathbb{E}[\left(\int_0^\infty |H|_s\diff |A|_s\right)^2]\coloneqq\lVert H\rVert_{L^2(M,A)}^2<\infty\right\}\;,
\end{align*}
    where \(|A|\) denotes the total variation process of the finite variation process \(A\).
\end{definition}
Then, as usual we define 
    \begin{equation}
        L(M,A)=\mathcal{L}(M,A)/\sim\;,
    \end{equation}
    where \(X\sim Y\) if \(\lVert X-Y\rVert_{L^2(M,A)}^2=0\).
Clearly, \(\mathcal{L}(X,A)\subset \mathcal{L}(X)\). The definition of \(\mathcal{L}(X,A)\) appears naturally when one tries to bound \(\lVert H\bullet X\rVert_{L^2}\), which is useful for our purpose by \cref{rem:sufficient condition for extension}. Indeed
...
\begin{equation}
    \lVert \vartheta\rVert_{L^2(M,A)}\leq C \lVert \vartheta\rVert_{L^2(M)}\,.
\end{equation}
A natural sufficient condition is given by the assumption that 
\begin{equation}
    A<<\langle M\rangle\;.
\end{equation}
wit predictable density \(\lambda\). We formulate it as a condition
\begin{definition}
    Let \(A\) and \(M\) be the finite variation part and the locally square integrable part respectively of the canonical decomposition of the semimartingale \(X\). The \(X\) satisfies the condition \(NA\) if there exists a predictable process \(\lambda_s\) such that
    \begin{equation}
        \diff As= \lambda_s\diff\langle X\rangle_s\quad\mathbb{P}-a.s.\forall t\in[0,\infty)\;.
    \end{equation}
\end{definition} 
If \(X\) satisfies \(NA\) then we have 
...
\end{equation}
Notice \(\int_0^{\infty}(\lambda_s)^2\diff [M]_s=\int_0^{\infty}|\lambda_s|\diff A_s\leq \int_0^{\infty}|\lambda_s|\diff |A|_s\). Imposing the additional assumption
\begin{equation}
    \lVert \int_0^{\infty}|\lambda_s|\diff |A|_s\rVert_{\infty}=\operatorname*{ess\sup}\int_0^{\infty}|\lambda_s|\diff |A|_s<\infty\;.
\end{equation}
We have the equivalence of norms on \(L^2(M)\) and \(L^2(M,A)\).
Thus, we have found one sufficient condition under which we get a generalization of the deep mean-variance hedging result \cref{thm:martingale setup, continuous deep variance hedging}. 
\begin{definition}\label{def:condition NA_strong}
    Assume \(X\) is a semimartingale with canonical decomposition \(X=X_0+M+A\). Then we say that \(X\) satisfies condition \(NA_{strong}\) if
    \begin{itemize}
        \item \(X\) satisfies condition \(NA\)\;,
        \item \(\operatorname*{ess\sup}\int_0^{\infty}|\lambda_s|\diff |A|_s<\infty\).
    \end{itemize}
\end{definition}
Furthermore, we define
\begin{definition}
Let \(A\) be a process of finite variation and \(|A|\) its total variation process. Then \(\vartheta\in L^2(A)\) if 
\begin{equation}
    \int_0^\infty |\vartheta_s|\diff |A|_s\in\mathcal{L}^2(\mathbb{P})\;.
\end{equation}
\end{definition}
In particular, notice that under conditions \cref{def:condition NA_strong} we have \(L^2(A)\subset L^2(M)\). 
\\\\
We have the following proposition:
\begin{proposition}
    Assume the \(X=X_0+M+A\) such that \(\exists \epsilon>0\) for which \(M\in\mathcal{H}^{2+\epsilon}\) and assume \(X\) satisfies condition \(NA_{strong}\). Let \(H\in L^2(\mathbb{P})\) be a real-valued \(\mathcal{F}_{\infty}\)-measurable random variable. Furthermore, assume \(\mathcal{F}_0\) to be trivial. Then:
\begin{equation}\label{eq:martingale setup, continuous deep variance hedging}
...
\end{proof}
We will now take a step back and study necessary and sufficient conditions for the equivalence of the norms on \(L^2(M)\) and \(L^2(M,A)\). For this we follow \parencite{DelbaenMonatSchachermayerSchweizerStricker1997WeightedNormInequalities}.
\textblue{I may have to redefine the norm on \(L^2(A)\) as it should match the one given in the Schweizer paper for the necessary and sufficient condition. In particular they differ in the multidimensional case}
