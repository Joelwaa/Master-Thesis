...
\\
For the following part, we do not consider the differentiation between constrained and unconstrained strategies; we also neglect trading costs, i.e., \(C_t=0\quad \forall t\geq 0\). The minimization problem we are we study now takes the form 
...
\end{align}
In this approach, we need to show that \(\lVert\cdot \rVert_{L^2(X)}\) is indeed a norm (or a semi-norm), that \(\mathcal{S}(\psi)\) is dense in \(L^2(X)\), and whether \(L^2(X)\) is rich enough to be interesting.
...
\begin{align*}
&\lVert H\rVert_{L^2(X)}^2\leq\mathbb{E}[\int_0^\infty (H_s)^2\diff [M]_s]+\mathbb{E}[\left(\int_0^\infty |H_s||\lambda_s|\diff[M]_s\right)^2]\\&\leq\mathbb{E}[\int_0^{\infty}(H_s)^2\diff [M]_s\cdot\int_0^{\infty}(\lambda_s)^2\diff [M]_s]
\\&
\leq\mathbb{E}[\left(\int_0^{\infty}(H_s)^2\diff [M]_s\right)^{p}]^{1/p}\cdot\mathbb{E}[\left(\int_0^{\infty}(\lambda_s)^2\diff [M]_s\right)^{q}]^{1/q}
\;,
\end{align*}
...
\begin{equation}
    \lVert H\rVert_{L^2(X)}\leq\lVert H\rVert_{L^2(X)}\operatorname*{ess\sup}\left(\int_0^{\infty}(\lambda_s)^2\diff [M]_s\right)\;.
\end{equation}
Notice \(\int_0^{\infty}(\lambda_s)^2\diff [M]_s=\int_0^{\infty}|\lambda_s|\diff |A|_s\). We impose the additional assumption
\begin{equation}
...
\begin{align}\label{eq:essential boundedness assumption}
    \mathbb{E}[\int_0^{\infty}H_s^2\diff[M]_s]\leq  \mathbb{E}[\int_0^{\infty}H_s^2\diff[M]_s]+  \mathbb{E}[\int_0^{\infty}|H_s||\lambda_s|\diff[M]_s]
\end{align}
we have that the norms on \(L^2(M)\) and \(L^2(X)\),under the condition that \(X = X_0+M+A\) with \([A<<[M]\) and \(M\in\mathcal{H}^{p+\epsilon}_0\) for some \(p\in[0,\infty)\) and \(\epsilon>0\), along with the additional assumption \cref{eq:essential boundedness assumption}, are equivalent. Because both spaces \(L^2(M)\) and \(L^2(X)\) are subspaces of the space of \(\mathcal
...
\begin{equation}
    L^2(M) =L^2(X)\;.
\end{equation}
Hence, density of \(\mathcal{E}(\psi)\) in \(L^2(M)\) is obviously equivalent to density of \(\mathcal{E}(\psi)\) in \(L^2(X)\) under the stated assumptions on \(M\) and \(X\). Hence, for every \(H\in L^2(X)\) there exists \((H_n)_{n\in\mathbb{N}}\subset \mathcal{E}(\psi)\) such that \(\lim_{n\to\infty}\lVert H-H_n\rVert_{L^2(X)}=0\). Then we have, by the observation \cref{eq:bounding the L^2 norm of the stochastic integral}, with \(T=\infty\) and \(H_s\coloneqq (H-H_n)_s\)
\begin{equation}
    \lVert (H-H^n)\bullet X\rVert_{L^2}
    \lesssim\lVert H-H^n\rVert_{L^2(X)}\;,
\end{equation}
thus
\begin{equation}
